...
trend_msg = emoji + "走勢圖是技術分析的基礎，投資者可結合圖表形態或技術指標進行交易決策。\n" \
             + emoji + "通過視覺化展示價格趨勢和技術指標，用戶可進行短期交易決策和分析市場趨勢。"
yield_msg = emoji + "通過量化收益數據，支援長期投資評估和資產比較。\n" \
             + emoji + "結合收益率和價格波動性，評估投資的風險回報比。"
kchart_msg = emoji + "每根K線代表一段時間的股價變化，由開盤、最高、最低和收盤價組成。\n" \
             + emoji + "收盤價高於開盤價 ➜ 畫紅K（上漲），反之 ➜ 畫綠K（下跌）。"
